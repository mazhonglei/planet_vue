                ...
                <div class="transfer-detail-home">
                    <div class="transfer-detail-home-in">
                        ...
                        <div class="transfer-detail-info">
                            <div>
                                <div>金额:</div>
                                <div>{{ obj.amount | roundOff(2) }} {{ $route.query.name }}</div>
                            </div>
                            <div>
                                <div>矿工费:</div>
                                <div>
                                    <div>{{ transaction.gasUsed * transaction.gasPrice / 10 ** 9 | roundOff(6) }} VNS
                                    </div>
                                    <div>≈Gas({{ transaction.gasUsed }})*GasPrice({{ transaction.gasPrice }} Gwei)</div>
                                </div>
                            </div>
                            <div>
                                <div>付款地址:</div>
...
<script>
    ...
    import {getTransaction, getTransactionReceipt} from "@/assets/js/etherjs/account";
    ...
    export default {
        ...
        methods: {
            showQrcode: function () {
                ...
                QRCode.toCanvas(canvas, this.obj.transactionHash, {
                    width: 80,
                    margin: 0
                }, function (error) {
                    if (error)
            ...
            getTransaction: async function () {
                this.transaction = await getTransaction(this.$route.query.transactionHash);
                let transaction = await getTransactionReceipt(this.$route.query.transactionHash);
                this.transaction.gasPrice = this.transaction.gasPrice.toString() / 10 ** 9;
                this.transaction.gasUsed = transaction.gasUsed.toString();
            }
        }
    }
</script>
